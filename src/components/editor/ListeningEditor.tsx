...
export default function ListeningEditor({ testId, onBack, onPreview }: ListeningEditorProps) {
  ...
  const [deleteAudioConfirm, setDeleteAudioConfirm] = useState<{ scope: 'test' | 'section' } | null>(null);
  const [editingBlock, setEditingBlock] = useState<any>(null);
  
  const testAudioInputRef = useRef<HTMLInputElement>(null);
  const sectionAudioInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleDeleteBlock = async () => {
    ...
    setIsProcessing(true);
    try {
      await DB.deleteBlock(testId, section.id, deleteConfirmBlockId);
  ...
  const handleUploadAudio = async (file: File, scope: 'test' | 'section') => {
    setIsProcessing(true);
    try {
      if (scope === 'test') {
        await DB.uploadTestAudio(testId, file);
      } else {
        const section = test.data[activeSectionIndex];
        await DB.uploadSectionAudio(testId, section.id, file);
      }
      await loadTestData();
    } catch (err) {
  ...
  const handleDeleteAudio = async () => {
    if (!deleteAudioConfirm) return;
    const { scope } = deleteAudioConfirm;
    setIsProcessing(true);
    try {
      if (scope === 'test') {
        await DB.updateTest(testId, { testAudio: null });
      } else {
        const section = test.data[activeSectionIndex];
        await DB.updateSection(testId, section.id, { sectionAudio: null });
      }
    ...
    <div className="flex flex-col h-full bg-[#f8fafc]">
      ...
      <div className="flex flex-grow overflow-hidden">
        ...
        <aside className="w-60 bg-white border-r border-[#e2e8f0] flex flex-col shrink-0 overflow-y-auto">
          {test.audioMode === 'whole' && (
            <div className="p-4 border-b border-[#e2e8f0] bg-[#f8fafc]">
              <h3 className="text-[11px] font-bold text-[#64748b] uppercase tracking-wider mb-3">Test Audio</h3>
              {test.audio?.url ? (
                <div className="space-y-2">
                  <div className="flex items-center gap-2 p-2 bg-white border border-[#e2e8f0] rounded-lg shadow-sm">
                    <Music size={14} className="text-[#2563eb]" />
                    <span className="text-[10px] font-medium truncate flex-grow">{test.audio.name}</span>
                  </div>
                  <div className="flex gap-2">
                    <button 
                      onClick={() => testAudioInputRef.current?.click()}
                      className="flex-grow py-1.5 border border-dashed border-[#cbd5e1] rounded-lg text-[10px] font-bold text-[#64748b] hover:bg-white transition-all"
                    >
                      Change Audio
                    </button>
                    <button 
                      onClick={() => setDeleteAudioConfirm({ scope: 'test' })}
                      className="p-1.5 border border-red-100 rounded-lg text-red-500 hover:bg-red-50 transition-all"
                      title="Delete Audio"
                    >
                      <Trash2 size={12} />
                    </button>
                ...
                <button 
                  onClick={() => testAudioInputRef.current?.click()}
                  className="w-full py-4 border-2 border-dashed border-[#e2e8f0] rounded-xl flex flex-col items-center justify-center gap-2 text-[#64748b] hover:border-[#2563eb] hover:text-[#2563eb] transition-all"
                >
                  <Upload size={20} />
                  <span className="text-[10px] font-bold uppercase">Upload Audio</span>
                </button>
              )}
              <input 
                type="file" 
                ...
                className="hidden" 
                accept="audio/*" 
                onChange={(e) => e.target.files?.[0] && handleUploadAudio(e.target.files[0], 'test')}
              />
            </div>
          )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-sm space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[11px] font-bold text-[#64748b] uppercase tracking-wider">Section Title</label>
                  <input 
                    type="text"
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea 
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-[#e2e8f0] bg-[#f8fafc] text-sm outline-none focus:border-[#2563eb] focus:bg-white transition-all min-h-[100px] resize-none"
                  />
                </div>
              </div>

              <div className="bg-white border border-[#e2e8f0] rounded-2xl p-6 shadow-sm">
                <h3 className="text-[11px] font-bold text-[#64748b] uppercase tracking-wider mb-4">Audio Asset</h3>
                {test.audioMode === 'whole' ? (
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-xl text-center">
                    <Headphones className="mx-auto text-slate-300 mb-2" size={24} />
                  ...
                  <div className="space-y-4">
                    {activeSection?.audio?.url ? (
                      <div className="space-y-3">
                        <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl">
                          <div className="flex items-center gap-2 mb-2">
                            <Music size={14} className="text-[#2563eb]" />
                            <span className="text-xs font-bold text-[#1e293b] truncate">{activeSection.audio.name}</span>
                          </div>
                          <audio src={activeSection.audio.url} controls className="w-full h-8" />
                        </div>
                        <div className="flex gap-2">
                          <button 
                            onClick={() => sectionAudioInputRef.current?.click()}
                            className="flex-grow py-2 border border-[#e2e8f0] rounded-lg text-xs font-bold text-[#64748b] hover:bg-[#f8fafc] transition-all"
                          >
                            Change Audio
                          </button>
                          <button 
                            onClick={() => setDeleteAudioConfirm({ scope: 'section' })}
                            className="p-2 border border-red-100 rounded-lg text-red-500 hover:bg-red-50 transition-all"
                            title="Delete Audio"
                          >
                            <Trash2 size={14} />
                          </button>
                      ...
                      <button 
                        onClick={() => sectionAudioInputRef.current?.click()}
                        className="w-full py-10 border-2 border-dashed border-[#e2e8f0] rounded-2xl flex flex-col items-center justify-center gap-3 text-[#64748b] hover:border-[#2563eb] hover:text-[#2563eb] transition-all"
                      >
                        <Upload size={28} />
                        <div className="text-center">
                          <span className="block text-xs font-bold uppercase tracking-wider">Upload Section Audio</span>
                          <span className="text-[10px] opacity-60 mt-1">MP3 or WAV files</span>
                        </div>
                      </button>
                    )}
                    <input 
                      type="file" 
                      ref={sectionAudioInputRef} 
                      className="hidden" 
                      accept="audio/*" 
                      onChange={(e) => e.target.files?.[0] && handleUploadAudio(e.target.files[0], 'section')}
                    />
                  </div>
                )}
              </div>
                    ...
                    {isAddMenuOpen && (
                      <>
                        ...
                        <motion.div 
                          ...
                          className="absolute right-0 top-full mt-2 w-56 bg-white border border-[#e2e8f0] rounded-xl shadow-xl z-20 overflow-hidden py-1"
                        >
                          {[
      ...
      <ConfirmationModal 
        isOpen={!!deleteAudioConfirm}
        onClose={() => setDeleteAudioConfirm(null)}
        onConfirm={handleDeleteAudio}
        title="Delete Audio File?"
        message={`Are you sure you want to delete the ${deleteAudioConfirm?.scope} audio? This action cannot be undone.`}
